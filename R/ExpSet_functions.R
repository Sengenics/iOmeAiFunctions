...
#' Add a Matrix to an ExpressionSet as a New Assay
#'
...
#'
#' @param ExpSet An ExpressionSet object to which the matrix will be added
#' @param matrix A matrix to add as a new assay. Must contain at least the same
...
#' @param name Character string specifying the name for the new assay. This will
#'   be used to access the assay later (e.g., via \code{assayData(ExpSet)[[name]]})
#'
#' @return An ExpressionSet object with the new matrix added as an assay. The
...
#'
#' @export
ExpSet_add_matrix_function <- function(ExpSet, matrix, name) {
	## ExpressionSet_functions.R ##
	
	# Extract current assay data as a list
	matrix = as.matrix(matrix)
	current_assay_data <- as.list(assayData(ExpSet))
	
	# Subset and reorder the matrix to match ExpressionSet features and samples
	# Rows: match feature names from ExpressionSet
	# Columns: match sample names from pData rownames
	current_assay_data[[name]] <- matrix[rownames(ExpSet), rownames(pData(ExpSet))]
	
	# Reconstruct the ExpressionSet with updated assay data
	assayData(ExpSet) <- do.call(assayDataNew, current_assay_data)
